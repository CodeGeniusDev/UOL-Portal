...
export default function StudentDashboard() {
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      content:
        "Hello! I am your UOL AI Assistant. Ask me anything — bus timings, hostel info, HODs, cafes, and more. 🎓",
    },
  ]);
  ...
  const handleSend = useCallback(async () => {
    ...
    const newMessage: Message = {
      role: "user",
      content: trimmed || "What is in this image?",
    };
    if (selectedImage) {
    ...
    try {
      ...
      if (data.response) {
        ...
        setMessages((prev) => [
          ...
          {
            role: "assistant",
            content: `⚠️ Error: ${data.error || "Unknown error. Please try again."}`,
          },
        ]);
      }
    } catch (error: any) {
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans">
      ...
      <aside className="w-72 bg-white border-r hidden md:flex flex-col p-6 shadow-sm">
        ...
        <div className="space-y-6 flex-1">
          ...
          <div className="bg-green-50 p-4 rounded-2xl border border-green-100">
            <p className="text-[10px] text-green-600 uppercase font-bold tracking-widest mb-1">
              Current Department
            </p>
          ...
          <nav className="space-y-2">
            {[
              ...
              <Button
                ...
                className={`w-full justify-start cursor-pointer rounded-xl px-4 py-6 transition-all ${activeTab === tab.id ? "font-bold text-green-700 bg-green-50 shadow-sm" : "text-gray-500 hover:bg-green-50 hover:text-green-700"}`}
              >
          ...
          <div className="pt-4 border-t">
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                {
                  icon: <Hotel className="w-4 h-4 text-orange-500" />,
                  label: "Hostels",
                  q: "Tell me about hostels at UOL",
                },
                {
                  icon: <Coffee className="w-4 h-4 text-purple-500" />,
                  label: "Cafes",
                  q: "What cafes are at UOL?",
                },
                {
                  icon: <Bus className="w-4 h-4 text-blue-500" />,
                  label: "Buses",
                  q: "What are the UOL bus routes?",
                },
                {
                  icon: <GraduationCap className="w-4 h-4 text-green-600" />,
                  label: "Faculty",
                  q: "List all UOL faculties",
                },
              ].map((item) => (
                <button
                  key={item.label}
        ...
        <div className="mt-auto pt-4 border-t">
          ...
          {uploadStatus && (
            <p className="text-[10px] mt-2 text-green-600 font-bold text-center">
              {uploadStatus}
            </p>
          )}
        </div>
          ...
          <header className="mb-4 flex items-center bg-amber-300 justify-between">
            <div>
              ...
              <p className="text-sm text-gray-400 font-medium">
                Trained on official UOL data · Speaks & Listens
              </p>
            </div>
            <div className="flex items-center gap-2">
              {isSpeaking && (
                <Button
                  ...
                  className="text-xs rounded-full border-red-200 text-red-500 hover:bg-red-50 cursor-pointer"
                >
            ...
            >
              {messages.map((m, i) => (
                ...
                >
                  ...
                  >
                    <div
                      className={`h-9 w-9 rounded-2xl flex items-center justify-center shrink-0 shadow-sm ${m.role === "user" ? "bg-green-600 text-white" : "bg-green-100 text-green-700"}`}
                    >
                      {m.role === "user" ? (
                        <User size={16} />
                      ) : (
                    ...
                    <div className="group relative flex flex-col gap-1">
                      ...
                      {m.role === "assistant" && (
                        <button
                          ...
                          className="self-start flex items-center gap-1 text-[10px] text-gray-400 hover:text-green-600 transition-colors font-bold px-1 cursor-pointer"
      ...
      "
            ...
            "
                      >
                        <span className="text-[12px] font-semibold text-green-700 whitespace-nowrap group-hover:text-green-800">
                          {q}
                        </span>
                      </button>
                    ))}
                  ...
                  <p className="text-[10px] text-gray-300 mt-2 px-1">
                    Tap any question to instantly ask the assistant
                  </p>
                </div>
              )}
            </div>
            ...
            {listenHint && (
              <div className="px-6 pb-1">
                <p
                  className={`text-xs font-bold px-3 py-2 rounded-xl ${isListening ? "bg-red-50 text-red-500" : "bg-green-50 text-green-600"}`}
                >
                  🎤 {listenHint}
                </p>
              </div>
            ...
            <div className="p-4 bg-white border-t">
              {selectedImage && (
                <div className="relative inline-block mb-3">
                  <img
                    src={selectedImage}
                    alt="Preview"
                    className="h-20 rounded-xl shadow-sm border border-gray-200"
                  />
                  <button
              ...
              <div className="flex gap-2 bg-gray-50 p-2 rounded-2xl border border-gray-100 focus-within:border-green-500 focus-within:ring-2 focus-within:ring-green-50 transition-all">
                <Input
                  ...
                  disabled={loading}
                  className="flex-1 border-none text-gray-700 bg-transparent focus-visible:ring-0 shadow-none text-sm font-medium"
                />
                ...
                <Button
                  ...
                  className="bg-green-600 hover:bg-green-700 rounded-xl shadow-md shadow-green-100 transition-all cursor-pointer"
                >
          ...
          <div className="grid gap-6 md:grid-cols-2 max-w-4xl">
            <Card className="border-none shadow-md rounded-3xl overflow-hidden">
              ...
              <CardContent className="p-6">
                ...
                <p className="text-xs text-gray-400 mt-4">
                  Register via OSA (EE1 Ground Floor) · Pay on SIS
                </p>
              </CardContent>
            </Card>
            ...
            <Card className="border-none shadow-md rounded-3xl overflow-hidden">
              ...
              <CardContent className="p-6">
                ...
                <p className="text-xs text-gray-400 mt-4">
                  24/7 Security · Wi-Fi · 3 Meals/Day
                </p>
              </CardContent>
            </Card>
              ...
              <CardContent className="p-6">
                <ul className="space-y-3 text-sm text-gray-700">
                  {[
                    ...
                    <li key={c} className="py-2 border-b last:border-0">
                      {c}
                    </li>
                  ))}
                </ul>
            ...
            <Card className="border-none shadow-md rounded-3xl overflow-hidden">
              ...
              <CardContent className="p-6">
                <ul className="space-y-3 text-sm text-gray-700">
                  {[
                    {
                      label: "Office of Student Affairs (OSA)",
                      value: "EE1, Ground Floor",
                    },
                    {
                      label: "University Counseling Center",
                      value: "Main Campus",
                    },
